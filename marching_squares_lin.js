...
function draw_curvature(position, x_init, y_init, x_fin, y_fin, scale, context, grid_size, context_height, isoval, func) {
    ...
    function y_lin(x, y, isoval, scale, func){
        ...
        parser.eval(func); 
        var val_low = parser.eval("f(" + x + "," + (y-scale) + ")");
        var val_high = parser.eval("f(" + x + "," + y + ")");
        var y_calc = y-scale + (scale)*((isoval - val_low)/(val_high - val_low));
        return y_calc;
    }
    function x_lin(x, y, isoval, scale, func)
    {
        ...
        parser2.eval(func);
        var val_low = parser2.eval("f(" + x + "," + y + ")");
        var val_high = parser2.eval("f(" + (x+scale) + "," + y + ")");
        var x_calc = x + (scale)*((isoval - val_low)/(val_high - val_low));
        return x_calc; 
    }
    function scrn_x(x, grid_size, x_init)
    {
        var scrn_x = grid_size * x - (grid_size * x_init) +50;
        return scrn_x;
    }
    function scrn_y(y, grid_size, y_init, context_height)
    {
        var scrn_y = context_height - 4 - (grid_size * y); //+ (grid_size * y_init);
        return scrn_y;
    }
        ...
        */
        ...
        switch(position[i][3]) {
            case 1:
                ...
                context.moveTo(scrn_x(position[i][0], grid_size, x_init), scrn_y(y_lin(position[i][0], position[i][1], isoval, scale, func), grid_size, y_init, context_height));
                context.lineTo(scrn_x(x_lin(position[i+1][0], position[i+1][1], isoval, scale, func)),scrn_y(position[i-1][1], grid_size, y_init, context_height))
                context.stroke();
                console.log(1);
                break;
            case 8:
                ...
                context.moveTo(scrn_x(x_lin(position[i][0], position[i][1], isoval, scale, func)), scrn_y(position[i][1], grid_size, y_init, context_height));
                context.lineTo(scrn_x(position[i][0], grid_size, x_init), scrn_y(y_lin(position[i][0], position[i][1], isoval, scale, func), grid_size, y_init, context_height));
                context.stroke();
                console.log(8);
                break;
            case 3: case 12:
                ...
                context.moveTo(scrn_x(position[i][0], grid_size, x_init),scrn_y(y_lin(position[i][0], position[i][1], isoval, scale, func), grid_size, y_init, context_height));
                context.lineTo(scrn_x(position[i][0], grid_size, x_init) + grid_scale, scrn_y(y_lin(position[i][0], position[i][1], isoval, scale, func), grid_size, y_init, context_height));
                context.stroke();
                console.log(312);
                break;
            case 6: case 9:
                ...
                context.moveTo(scrn_x(x_lin(position[i+1][0], position[i+1][1], isoval, scale, func)),scrn_y(position[i][1], grid_size, y_init, context_height));
                context.lineTo(scrn_x(x_lin(position[i+1][0], position[i+1][1], isoval, scale, func)),scrn_y(position[i+1][1], grid_size, y_init, context_height));
                context.stroke();
                console.log(69);
                break;
            ...
            case 2:
                ...
                context.moveTo(scrn_x(position[i][0], grid_size, x_init) + grid_scale, scrn_y(y_lin(position[i][0] + scale, position[i][1], isoval, scale, func), grid_size, y_init, context_height));
                context.lineTo(scrn_x(x_lin(position[i][0], position[i-1][1], isoval, scale, func), grid_size, x_init), scrn_y(position[i-1][1], grid_size, y_init, context_height));
                context.stroke();
                console.log(2);
                /*console.log(scrn_x(position[i][0], grid_size, x_init) + grid_scale);
                console.log(scrn_y(y_lin(position[i][0] + scale, position[i][1], isoval, scale, func), grid_size, y_init, context_height));
                
                console.log(scrn_x(x_lin(position[i][0], position[i-1][1], isoval, scale, func), grid_size, x_init));
                console.log(y_init);
                //console.log(position[i][1]-2);
                console.log(scrn_y(position[i-1][1], grid_size, y_init, context_height));*/
                break;
